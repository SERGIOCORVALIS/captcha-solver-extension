...
class ServiceWorker {
  ...
  private async init(): Promise<void> {
    ...
    chrome.storage.onChanged.addListener((changes) => {
      if (changes.extension_config) {
        const newConfig = changes.extension_config.newValue as
          | import("../shared/types/config.types").ExtensionConfig
          | undefined;
  ...
  private async handleSolveCaptcha(
    ...
    const safeSendResponse = (response: MessageResponse) => {
      if (!responseSent) {
        try {
          ...
          logger.debug(
            "Error sending response in handleSolveCaptcha (channel may be closed)",
            {
              error: error instanceof Error ? error.message : "Unknown error",
            },
          );
    ...
    try {
      ...
      if (
        detection.type === CaptchaType.IMAGE_CAPTCHA &&
      ...
      logger.info(
        "[CAPTCHA Solver] [Background] ===== STARTING CAPTCHA SOLVE =====",
      );
      logger.info("[CAPTCHA Solver] [Background] Detection:", {
        type: detection.type,
      ...
      logger.info(
        "[CAPTCHA Solver] [Background] SolverAPI initialized:",
        !!this.solverAPI,
      );
      ...
      logger.info(
        "[CAPTCHA Solver] [Background] Sending request to solver API...",
      );
      logger.info("[CAPTCHA Solver] [Background] Request details:", {
        type: request.type,
      ...
      const result = await measurePerformance("solve-captcha", () =>
        this.requestQueue.enqueue(
          () => {
            logger.info(
              "[CAPTCHA Solver] [Background] Calling solverAPI.solve()...",
            );
            logger.info("[CAPTCHA Solver] [Background] SolverAPI config:", {
      ...
      logger.info(
        `[CAPTCHA Solver] [Background] Solver API response received (took ${duration}ms):`,
        {
      ...
      if (result.success) {
        ...
        logger.info(
          "[CAPTCHA Solver] [Background] Checking for token in result:",
          {
            hasToken: !!result.token,
            tokenLength: result.token?.length || 0,
            success: result.success,
        ...
        if (result.token && result.token.length > 0) {
          logger.info(
            "[CAPTCHA Solver] [Background] ✓ Token received from server, injecting...",
          );
          ...
          console.warn(
            "[CAPTCHA Solver] [Background] ✗ Server did not return token!",
            {
              success: result.success,
              error: result.error,
            },
        ...
        logger.info(
          "[CAPTCHA Solver] [Background] Sending success response to content script",
          {
            hasToken: !!result.token,
            tokenLength: result.token?.length || 0,
          },
        ...
        try {
          ...
          logger.debug(
            "sendResponse failed but result already sent via tabs.sendMessage",
          );
        }
      } else {
  ...
  private async injectToken(
    ...
    if (targetTabId) {
      try {
        logger.info(
          "[CAPTCHA Solver] [Background] Sending INJECT_TOKEN message to tab",
          {
            tabId: targetTabId,
            type: detection.type,
        ...
        logger.info("Sending INJECT_TOKEN message to tab", {
          tabId: targetTabId,
        });
        ...
        logger.info(
          "[CAPTCHA Solver] [Background] ✓ INJECT_TOKEN message sent successfully",
        );
  ...
  private async handleResetStatistics(
    sendResponse: (response: MessageResponse) => void,
  ): Promise<void> {
