...
export class SolverAPI {
  ...
  async solve(request: SolveRequest): Promise<SolveResponse> {
    console.log('[CAPTCHA Solver] [SolverAPI] solve() called:', {
      type: request.type,
    ...
    if (request.type !== CaptchaType.IMAGE_CAPTCHA && !isValidSiteKey(request.siteKey)) {
      return {
        success: false,
    ...
    if (!this.config.provider || this.config.provider === 'none') {
      if (request.type === CaptchaType.IMAGE_CAPTCHA && !this.config.useLocalSolver) {
        throw new Error('No solver configured for image CAPTCHA. Enable local solver or configure API.');
      }
      throw new Error('No API provider configured');
    }

    // Validate API key for external providers
    if (this.config.provider !== 'custom' && !isValidApiKey(this.config.apiKey || '')) {
      return {
        success: false,
    ...
    if (this.config.provider === 'custom' && !isValidUrl(this.config.endpoint || '')) {
      return {
        success: false,
    ...
    console.log('[CAPTCHA Solver] [SolverAPI] Routing to provider:', this.config.provider);
    return retry(
      () => {
        switch (this.config.provider) {
          case '2captcha':
            console.log('[CAPTCHA Solver] [SolverAPI] Using 2Captcha provider');
            return this.solve2Captcha(request);
          case 'anticaptcha':
            console.log('[CAPTCHA Solver] [SolverAPI] Using AntiCaptcha provider');
            return this.solveAntiCaptcha(request);
          case 'capsolver':
            console.log('[CAPTCHA Solver] [SolverAPI] Using CapSolver provider');
            return this.solveCapSolver(request);
          case 'custom':
            console.log('[CAPTCHA Solver] [SolverAPI] Using custom provider, calling solveCustom()...');
            return this.solveCustom(request);
          default:
            console.error('[CAPTCHA Solver] [SolverAPI] Unsupported provider:', this.config.provider);
  ...
  private async solveLocalImage(request: SolveRequest): Promise<SolveResponse> {
    ...
    try {
      const result = await this.localImageSolver.solveImageFromBase64(request.imageBase64);
   ...
   */
  private async solveAntiCaptcha(request: SolveRequest): Promise<SolveResponse> {
    const apiKey = this.config.apiKey;
    if (!apiKey) {
      return { success: false, error: 'API key not configured' };
    }
    ...
    try {
      ...
      if (createResult.errorId !== 0) {
        return { success: false, error: createResult.errorDescription || 'Failed to create task' };
      }
  ...
  private buildAntiCaptchaTaskData(request: SolveRequest): Record<string, unknown> {
  ...
  private buildCapSolverTaskData(request: SolveRequest): Record<string, unknown> {
    switch (request.type) {
      case CaptchaType.RECAPTCHA_V2:
        return {
  ...
  private async solveCustom(request: SolveRequest): Promise<SolveResponse> {
    console.log('[CAPTCHA Solver] [SolverAPI] solveCustom called:', {
      type: request.type,
    ...
    try {
      ...
      if (!this.config.apiKey || this.config.apiKey.trim().length === 0) {
        ...
        return {
          success: false,
          error: 'API key is not configured. Please set the API key in extension settings.',
        };
      }
      ...
      const healthCheckUrl = this.config.endpoint.replace('/api/solve', '/health');
      try {
      ...
      console.log('[CAPTCHA Solver] [SolverAPI] Timeout configuration:', {
      ...
      try {
        ...
        const timeoutId = setTimeout(() => {
          console.error('[CAPTCHA Solver] [SolverAPI] ⚠️ TIMEOUT TRIGGERED after', timeout / 1000, 'seconds');
          controller.abort();
        }, timeout);
        
        console.log('[CAPTCHA Solver] [SolverAPI] Sending fetch request to:', this.config.endpoint);
        console.log('[CAPTCHA Solver] [SolverAPI] Request body:', {
          type: request.type,
          siteKey: request.siteKey?.substring(0, 10) + '...',
          pageUrl: request.pageUrl?.substring(0, 50) + '...',
          hasImage: !!request.imageBase64,
        });
        console.log('[CAPTCHA Solver] [SolverAPI] Request started at:', new Date().toISOString());
        console.log('[CAPTCHA Solver] [SolverAPI] API Key being sent:', {
          hasKey: !!this.config.apiKey,
          keyLength: this.config.apiKey?.length || 0,
          keyPrefix: this.config.apiKey ? this.config.apiKey.substring(0, 4) + '...' : 'N/A',
        });
        ...
        console.log('[CAPTCHA Solver] [SolverAPI] Response received at:', new Date().toISOString());
        console.log('[CAPTCHA Solver] [SolverAPI] Response status:', {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          try {
            ...
            if (errorData.error === 'Invalid API key' || errorData.error === 'API key required') {
              errorMessage = 'Invalid API key. Please check your API key in the extension settings and ensure it matches the server API_KEY in .env file.';
            } else {
          ...
          errorMessage = 'API endpoint not found. Please check the endpoint URL in extension settings.';
      ...
      console.log('[CAPTCHA Solver] [SolverAPI] Response data:', {
      ...
      if (!data.success) {
        console.error('[CAPTCHA Solver] [SolverAPI] Server returned error:', data.error);
      } else if (!data.token || data.token.length === 0) {
        console.warn('[CAPTCHA Solver] [SolverAPI] Server returned success but no token!');
      } else {
        console.log('[CAPTCHA Solver] [SolverAPI] ✓ Token received successfully!');
      }
