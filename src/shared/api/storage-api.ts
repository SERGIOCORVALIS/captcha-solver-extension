...
export class StorageAPI {
          ...
          if (result[STORAGE_KEYS.CONFIG]) {
            ...
            const config: ExtensionConfig = {
              ...
              apiConfig: {
                ...
                apiKey: storedConfig.apiConfig?.provider === 'custom' && storedConfig.apiConfig?.apiKey !== DEFAULT_CONFIG.apiConfig.apiKey
                  ? DEFAULT_CONFIG.apiConfig.apiKey // Use default key for custom provider
                  : (storedConfig.apiConfig?.apiKey || DEFAULT_CONFIG.apiConfig.apiKey),
                ...
                timeout: Math.max(
                  storedConfig.apiConfig?.timeout ?? DEFAULT_CONFIG.apiConfig?.timeout ?? 600000,
            ...
            if (config.apiConfig?.apiKey && isEncrypted(config.apiConfig.apiKey)) {
  ...
  static async updateConfig(config: Partial<ExtensionConfig>): Promise<void> {
    return new Promise((resolve, reject) => {
      chrome.storage.sync.get(STORAGE_KEYS.CONFIG, (result) => {
        const currentConfig = (result[STORAGE_KEYS.CONFIG] as ExtensionConfig) || DEFAULT_CONFIG;
        
        // Encrypt API key before storing
        const apiConfig = config.apiConfig ? { ...config.apiConfig } : currentConfig.apiConfig;
  ...
  static async getStatistics(): Promise<ExtensionConfig['statistics']> {
    return new Promise((resolve) => {
      chrome.storage.sync.get([STORAGE_KEYS.CONFIG, STORAGE_KEYS.STATISTICS], (result) => {
        const config = (result[STORAGE_KEYS.CONFIG] as ExtensionConfig) || DEFAULT_CONFIG;
        const stats = (result[STORAGE_KEYS.STATISTICS] as ExtensionConfig['statistics']) ||
          config.statistics;
        resolve(stats);
      });
    });
  ...
  static async updateStatistics(
    updater: (stats: ExtensionConfig['statistics']) => ExtensionConfig['statistics']
  ): Promise<void> {
    return new Promise((resolve, reject) => {
