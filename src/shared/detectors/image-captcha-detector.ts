...
export class ImageCaptchaDetector extends BaseDetector {
  ...
  async detect(): Promise<CaptchaDetectionResult | null> {
    ...
    const hasRecaptcha = document.querySelector('.g-recaptcha, [data-sitekey], .recaptcha-checkbox, iframe[src*="recaptcha"]');
    const hasHcaptcha = document.querySelector('.h-captcha, .hcaptcha-box, iframe[src*="hcaptcha"]');
    ...
    if (image) {
      ...
      if (src.includes('recaptcha') || src.includes('hcaptcha') || src.includes('gstatic.com') || 
          className.includes('recaptcha') || className.includes('hcaptcha')) {
        return null;
      }
      ...
      if (!isCaptchaImage && input) {
        ...
        const distance = Math.abs(imageRect.top - inputRect.top) + Math.abs(imageRect.left - inputRect.left);
    ...
    const hasCaptchaInput = inputElement && (
    ...
    const confidence = this.calculateConfidence(elementsFound, 3) * (isCaptchaImage ? 1 : 0.7);
